...
int numCities = 7;
float citySize = 10, pathSize = 2, bestPathSize = 4;

boolean finished = false;
int totalPermutations = factorial(numCities), calculatedPermutations = 0;
float optimalRoute, percentCompleted;
int[] currentPath = new int[numCities], bestPath = new int[numCities];
...
//float citySize = 10, record, percent, seconds;
...
//  fill(cityColour);
//  for (int i = 0; i < city.length; i++) {
//    ellipse(city[i].x, city[i].y, citySize, citySize);
//  }
//  noFill();
//  if (!finished) {
//    stroke(pathColour);
...
//  beginShape();
//  for (int i = 0; i < order.length; i++) {
//    vertex(city[bestPath[i]].x, city[bestPath[i]].y);
//  }
//  endShape();
